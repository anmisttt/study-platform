...
describe("room owner credentials and progress", () => {
  it("lets guests grade and transcribe using the owner's key, never the initiator's", async () => {
    ...
    const check = `/rooms/${id}/questions/theory-0/check`;
    ...
    expect(summaries[0].continueQuestionRef).toBe("theory-1");
  });
  it("does not fall back for missing, removed or legacy keys, and broadcasts availability", async () => {
    ...
    const check = () => api(`/rooms/${id}/questions/theory-0/check`, "POST", { answer: "Answer", baseRevision: 0 });
  ...
  it("rejects concurrent checks before charging and cannot restore a deleted room", async () => {
    ...
    const path = `/rooms/${id}/questions/theory-0/check`;
    const first = api(path, "POST", { answer: "Answer", baseRevision: 0 }); await startedPromise;
    expect(status).toHaveBeenCalledWith(id, "theory-0", true);
    const duplicate = await api(path, "POST", { answer: "Answer", baseRevision: 0 });
    expect((await duplicate.json()).code).toBe("ALREADY_CHECKING");
    const alias = await api(`/rooms/${id}/questions/theory-00/check`, "POST", { answer: "Alias", baseRevision: 0 });
    expect((await alias.json()).code).toBe("ALREADY_CHECKING");
    expect((await api(`/rooms/${id}/questions/theory-1/check`, "POST", { answer: "Independent question", baseRevision: 0 })).status).toBe(200);
    ...
    expect(status).toHaveBeenLastCalledWith(id, "theory-0", false);
  ...
  it.each([[401, "invalid_api_key", "OWNER_KEY_INVALID"], [429, "insufficient_quota", "LLM_QUOTA_EXCEEDED"], [404, "model_not_found", "LLM_MODEL_UNAVAILABLE"], [500, "server_error", "LLM_UNAVAILABLE"]])("maps provider failure %s without exposing credentials", async (status, code, expectedCode) => {
    ...
    const response = await api(`/rooms/${id}/questions/theory-0/check`, "POST", { answer: "Answer", baseRevision: 0 });
    const payload = await response.json();
    ...
    expect((await api(`/rooms/${id}/questions/theory-0/check`, "POST", { answer: "Answer", baseRevision: 0 })).status).toBe(200);
    expect((await api(`/rooms/${id}/questions/theory-0/check`, "POST", { answer: "Stale answer", baseRevision: 0 })).status).toBe(409);
    expect(services.grade).toHaveBeenCalledTimes(2);
  });
});
