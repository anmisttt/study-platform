...
- `POST /rooms/:roomId/questions/:questionRef/check` with `{ "answer": "...", "baseRevision": 0 }` — public, using the owner's key; question refs use one-based ordinals (`theory-1`, `practice-1`, etc.). Returns `rating`, `comment`, `revision`. Concurrent checks of the same question are rejected before the LLM request.
